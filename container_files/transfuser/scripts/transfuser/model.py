...
class LidarCenterNet(nn.Module):
    ...
    def __init__(self, config, device, backbone, image_architecture='resnet34', lidar_architecture='resnet18', use_velocity=True):
        ...
        print("Using backbone", self.backbone)
    ...
    def forward_ego(self, rgb, lidar_bev, target_point, target_point_image, ego_vel, bev_points=None, cam_points=None, save_path="/workspace/viz/", expert_waypoints=None,
    ...
    def draw_target_point(self, target_point, image, color = (255, 255, 255)):
        ...
        point[1] += int(self.config.lidar_resolution_height / 2.0) #Might be LiDAR width
        point = point.astype(np.int32)
    ...
    def visualize_model_io(self, save_path, step, config, rgb, lidar_bev, target_point,
                        pred_wp, pred_bev, pred_semantic, pred_depth, device,
        ...
        if config.multitask:
            ...
            cv2.imwrite(str(save_path + ("/%d_depth_and_semantic_pred.png" % frame_id)), ds_image)
        ...
        if (not (gt_bboxes is None)):
            ...
            lidar_panel = self.draw_bboxes(rotated_bboxes_gt, lidar_panel, color=(0, 255, 0), brake_color=(0, 255, 128))

        rotated_bboxes = []
        # for bbox in bboxes.detach().cpu().numpy():
        #     bbox = self.get_rotated_bbox(bbox[:7])
        #     rotated_bboxes.append(bbox)
        lidar_panel = self.draw_bboxes(rotated_bboxes, lidar_panel, color=(255, 0, 0), brake_color=(0, 255, 255))
        ...
        if not expert_waypoints is None:
            bev_image = self.draw_waypoints(label[0], expert_waypoints[i:i+1, -1:], bev_image, color=(0, 0, 255))
