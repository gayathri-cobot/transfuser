...
TRAINABLE_GROUPS = {
    'trunk': ['_model', 'point_pillar_net'],
    'wp':    ['join', 'decoder', 'output'],
    'bev':   ['pred_bev'],
    'seg':   ['seg_decoder'],
    'depth': ['depth_decoder'],
}

# The loss each head is responsible for. The trunk has no loss of its own, it is trained by all of them.
GROUP_LOSSES = {
    'wp':    'loss_wp',
    'bev':   'loss_bev',
    'seg':   'loss_semantic',
...
def parse_trainable(spec):
    """Parses the --trainable argument into a set of group names."""
    if (spec == 'all'):
        return set(TRAINABLE_GROUPS.keys())

    groups = set(name.strip() for name in spec.split(',') if name.strip())
    unknown = groups - set(TRAINABLE_GROUPS.keys())
    if (len(unknown) > 0):
        raise ValueError("Unknown --trainable group(s): %s. The options are: all, %s"
                         % (', '.join(sorted(unknown)), ', '.join(sorted(TRAINABLE_GROUPS.keys()))))
    if (len(groups) == 0):
        raise ValueError("--trainable selected no groups, there would be nothing to optimize.")
    return groups


def freeze_groups(model, trainable):
    """
    Sets requires_grad=False on every group that is not in trainable and returns the list of frozen
    modules. Call this on the unwrapped model, after any SyncBatchNorm conversion (which rebuilds the
    norm layers) and before wrapping in DistributedDataParallel, so that DDP's reducer never sees the
    frozen parameters. The returned modules additionally have to be kept in eval mode during training,
    otherwise their BatchNorm running stats keep updating on every forward pass and the frozen part of
    the network would still drift.
    """
    frozen_modules = []
    for group, module_names in TRAINABLE_GROUPS.items():
        if (group in trainable):
            continue
        for module_name in module_names:
            module = getattr(model, module_name, None)
            if (module is None): # Not built under this config, e.g. point_pillar_net or the multitask decoders.
                continue
...
def main():
    ...
    if ((parallel == True) and (bool(args.sync_batch_norm) == True)):
        # Synchronizing the Batch Norms increases the Batch size with which they are compute by *num_gpus
        model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)

    trainable_groups = parse_trainable(args.trainable)
    ...
    # Filter the trainable parameters
    trainable_params = [p for p in model.parameters() if p.requires_grad]
    if (len(trainable_params) == 0):
        raise ValueError("--trainable %s left no trainable parameters. Note that seg and depth only "
