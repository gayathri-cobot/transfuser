"""Bag preprocessing, distance-gated variant.

Same outputs as data_preprocess.py, but the reference tick is /tf rather than
the front camera: a frame is written only once the robot has moved --min-dist
metres since the last one. That gives frames spaced by distance travelled
instead of by time, so a run that idles doesn't fill the dataset with
duplicates. Front camera only.

Used directly:
    python data_preprocess_distance.py /workspace/bag_data/scenario_1/<run>

or, in parallel, through run_prep.py -> prep_extract.py (extraction) and
prep_segment.py (batched segmentation), which import the helpers below.
"""
import argparse
import glob
import json
...
#Topic details
# This variant is front-camera only: the reference tick is /tf (distance
# gated), not a camera topic, so the left/right streams are never synced
# against it. REF_SIDE is the subdirectory rgb/ and depth/ frames land in, and
# is what prep_segment.py globs for.
REF_SIDE = 'front'
...
def _resolve_candidates(type_map, candidates):
    """First of `candidates` actually recorded in the bag, or None.

    Lets one logical stream have several recorded names (plain vs throttled)
    without every call site having to know which one this bag used.
    """
    for candidate in candidates:
        resolved = _resolve_topic(type_map, candidate)
...
def find_first_last_cmd(bag_path):
    """Return the first and last map->base_link pose in a bag, and the yaw of
    travel between them, or (None, None, None) if the bag has no usable tf.

    /tf is multiplexed: one message carries whichever edges of the tf tree were
    published at that instant, and map->base_link is never one of them (these
    bags publish map->odom and odom->base_link separately). So the pose has to
    come out of a tf2 buffer, as save_synced_frames does - msg.transforms[0]
    would give an arbitrary edge, in the odom frame rather than the map frame
    that closest_command_point compares against.
    """
    buffer = tf2_ros.Buffer()
    first_cmd, last_cmd = None, None
    for t, msg, msg_type_name, topic_name in _iter_messages_multi(
            bag_path, [TF_STATIC_TOPIC, TF_TOPIC]):
        ...
        try:
            ...
            continue  # tf tree not connected yet
        if first_cmd is None:
            first_cmd = (t, pose, msg_type_name)
        last_cmd = (t, pose, msg_type_name)

    if first_cmd is None or last_cmd is None:
        return None, None, None

    # Direction of motion, not the robot's final heading: it can be turning at
    # the end of the run, and yaw here decides which axis case applies below.
    yaw = math.atan2(
        last_cmd[1].transform.translation.y - first_cmd[1].transform.translation.y,
        last_cmd[1].transform.translation.x - first_cmd[1].transform.translation.x)
    return first_cmd, last_cmd, yaw

def generate_command_points(bag_path):
    """Synthesise four waypoints along the start->end direction of a run.

    The fallback for bags with no route XML: without waypoints,
    closest_command_point() has nothing to return and every trajectory record
    would be missing its x_command/y_command.
    """
    first_cmd, last_cmd, yaw = find_first_last_cmd(bag_path)
    if first_cmd is None or last_cmd is None:
        return []

    dist = math.sqrt((last_cmd[1].transform.translation.x - first_cmd[1].transform.translation.x) ** 2 +  (last_cmd[1].transform.translation.y - first_cmd[1].transform.translation.y) ** 2)
    step = dist/4

    cmd_points = []

    # yaw is radians, so comparing it against 0/90/180 directly never matches.
    # Compare in degrees with a tolerance, since two recorded endpoints never
    # line up on an exact multiple of 90. Snapping to an axis is the point of
    # these cases: it cancels the small lateral offset between start and end.
    yaw_deg = math.degrees(yaw)

    def _heading_is(target_deg, tol_deg=5.0):
        return abs((yaw_deg - target_deg + 180) % 360 - 180) <= tol_deg

    # i starts at 1: i=0 is the start pose, which is the <start> spawn pose and
    # not a waypoint. With the endpoint appended below, the four points land at
    # step, 2*step, 3*step and dist.
    if _heading_is(0):
        for i in range(1, 4):
            x_val = first_cmd[1].transform.translation.x + i * step
            y_val = first_cmd[1].transform.translation.y
            cmd_points.append({'x': x_val, 'y': y_val, 'yaw': yaw})

    elif _heading_is(180):
        for i in range(1, 4):
            x_val = first_cmd[1].transform.translation.x - i * step
            y_val = first_cmd[1].transform.translation.y
            cmd_points.append({'x': x_val, 'y': y_val, 'yaw': yaw})
        ...
        for i in range(1, 4):
            x_val = first_cmd[1].transform.translation.x
            y_val = first_cmd[1].transform.translation.y + i * step
            cmd_points.append({'x': x_val, 'y': y_val, 'yaw': yaw})
        ...
        # Not axis aligned: step along the start->end direction itself.
        for i in range(1, 4):
            x_val = first_cmd[1].transform.translation.x + i * step * math.cos(yaw)
            y_val = first_cmd[1].transform.translation.y + i * step * math.sin(yaw)
            cmd_points.append({'x': x_val, 'y': y_val, 'yaw': yaw})

    cmd_points.append({'x': last_cmd[1].transform.translation.x, 'y': last_cmd[1].transform.translation.y, 'yaw': yaw})
    return cmd_points

def save_synced_frames(bag_path, type_map, counts, output_path,
                        ref_side=REF_SIDE, max_dt_ns=1e7, min_dist_m=0.5):
    ...
    # rgb/<side> and depth/<side>, not just rgb/ and depth/: the frames below are
    # written under the side subdirectory, and that is also where prep_segment.py
    # looks for its input.
    os.makedirs(os.path.join(output_path, 'rgb', ref_side), exist_ok=True)
    os.makedirs(os.path.join(output_path, 'depth', ref_side), exist_ok=True)
    ...
    depth_topics = _resolve_candidates(type_map, DEPTH_TOPIC_CANDIDATES)
    camera_topic = _resolve_candidates(type_map, CAMERA_TOPIC_CANDIDATES)

    print(depth_topics, camera_topic)

    lidar_topic = _resolve_candidates(type_map, LIDAR_TOPIC_CANDIDATES)

    # Nothing is written for a tick without both rgb and depth, so a bag missing
    # either produces an empty output tree; say so rather than reading it all.
    if camera_topic is None:
        print(f"[warn] no front camera topic in this bag "
              f"(tried {', '.join(CAMERA_TOPIC_CANDIDATES)}) — nothing to sync")
        return
    if depth_topics is None:
        print(f"[warn] no front depth topic in this bag "
              f"(tried {', '.join(DEPTH_TOPIC_CANDIDATES)}) — nothing to sync")
        return
    ...
    for t, msg, msg_type_name, topic_name in _iter_messages_multi(bag_path, topics):
        ...
        # command_pts is empty when the bag has neither a route XML nor enough tf
        # to synthesise waypoints; record nulls rather than killing the run.
        cmd_pt = (closest_command_point(command_pts, robot_x, robot_y, robot_yaw)
                  if command_pts else {'x': None, 'y': None})
...
def find_command_point(bag_path):
    ...
    Falls back to generate_command_points() when there is no usable route XML,
    so a bag whose scenario has no route definitions still gets processed
    ...
    try:
        ...
        print("Generating waypoints instead")
        return generate_command_points(bag_path)
...
def save_camera_data(bag_path, type_map, counts, output_path, side=REF_SIDE):
    """Save every front rgb + depth frame, unsynced, to disk.

    Not part of process_data() - save_synced_frames() covers the pipeline path.
    Kept for one-off inspection of a bag's raw camera streams.
    """
    #make RGB directory
    ...
    cam_topic = _resolve_candidates(type_map, CAMERA_TOPIC_CANDIDATES)
    depth_topic = _resolve_candidates(type_map, DEPTH_TOPIC_CANDIDATES)
...
def save_segmented_images(bag_path, type_map, counts, output_path):
    ...
    rgb_dir = os.path.join(output_path, 'rgb', REF_SIDE)
    segmented_dir = os.path.join(output_path, 'segmented', REF_SIDE)
